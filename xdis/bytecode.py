...
from xdis.util import better_repr, get_code_object, code2num, num2code, format_code_info
...
def _get_const_info(const_index, const_list):
    """Helper to get optional details about const references
       ...
       Returns the dereferenced constant and its better_repr() if the constant
       list is defined.
       Otherwise returns the constant index and its better_repr().
    """
    argval = const_index
    if const_list is not None:
        argval = const_list[const_index]
    return argval, better_repr(argval)
...
class Instruction(_Instruction):
    ...
    def disassemble(
        self, lineno_width=3, mark_as_current=False, asm_format=False, show_bytes=False
    ):
...
class Bytecode(object):
    ...
    def disassemble_bytes(
        ...
        for instr in get_instructions_bytes(
            ...
            file.write(
                instr.disassemble(
                    lineno_width, is_current_instr, asm_format, show_bytes
                )
                + "\n"
            )
            pass
